...
const orderController = () => {
    return {
        ...
        createOrder: async (req, res) => {
            ...
            const addedOrder = await createOrder(req.body, res)
            res.status(200).json(addedOrder);
        },

        // Find single product by ID 
        getSingleOrder: async (req, res) => {
            const order = getSingleOrder({ _id: mongoose.Types.ObjectId(req.params.id) }, res)
            res.status(200).json(order)
        },

        // Get all orders
        getAllOrders: async (req, res) => {
            const orders = await getAllOrders(res)
            res.status(200).json(orders)
        },

        // Update single order by ID
        updateOrder: async (req, res) => {
            const validation = orderValidation(req.body, res)
        ...
        deleteOrder: async (req, res) => {
            const deletedOrder = await deleteOrder({ _id: mongoose.Types.ObjectId(req.params.id) }, res)
