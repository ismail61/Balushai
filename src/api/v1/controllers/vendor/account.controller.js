...
function accountController() {
    return {
        getSellerAccountInfo: async (req, res) => {
            const vendor = await getVendorSellerAccountInfo({ _id: req.user?._id }, res);
        ...
        updateSellerAccountInfo: async (req, res) => {
            ...
            const updatedVendor = await updateSellerAccountInfo(req.user?._id, refactor_data, res);
            res.status(200).json(updatedVendor?.seller_account);
        },
        getBusinessInfo: async (req, res) => {
            const vendor = await getVendorBusinessInfo({ _id: req.user?._id }, res);
            res.status(200).json(vendor);
        },
        updateBusinessInfo: async (req, res) => {
            ...
            const updatedVendor = await updateVendorBusinessInfo(req.user?._id, refactor_data, res);
            res.status(200).json(updatedVendor?.business_information);
        },
        getBankInfo: async (req, res) => {
            const vendor = await getVendorBankInfo({ _id: req.user?._id }, res);
            res.status(200).json(vendor);
        },
        updateBankInfo: async (req, res) => {
            ...
            const updatedVendor = await updateVendorBankInfo(req.user?._id, refactor_data, res);
            res.status(200).json(updatedVendor?.bank_account);
        },
        getWarehouseAddress: async (req, res) => {
            const vendor = await getVendorWarehouseAddress({ _id: req.user?._id }, res);
            res.status(200).json(vendor);
        },
        updateWarehouseAddress: async (req, res) => {
            ...
            const updatedVendor = await updateVendorWarehouseAddress(req.user?._id, refactor_data, res);
            res.status(200).json(updatedVendor?.warehouse_address);
        },
        getReturnAddress: async (req, res) => {
            const vendor = await getVendorReturnAddress({ _id: req.user?._id }, res);
            res.status(200).json(vendor);
        },
        updateReturnAddress: async (req, res) => {
            ...
            const updatedVendor = await updateVendorReturnAddress(req.user?._id, refactor_data, res);
            res.status(200).json(updatedVendor?.return_address);
        },
        getSellerLogo: async (req, res) => {
            const img = await getVendorAccountLogo({ _id: req.user?._id }, res);
        ...
        getAccountPhoto: async (req, res) => {
            const img = await getVendorAccountPhoto({ _id: req.user?._id }, res);
            res.status(200).json(img);
        },
        ...
        changePassword: async (req, res) => {
            ...
            await findVendorByIDAndUpdate(vendor._id, { password: new_password }, res);
