...
function voucherController() {
    return {
        ...
        createVoucher: async (req, res) => {
            ...
            const addedVoucher = await createVoucher(req.body, req.admin?._id, res);
        ...
        getAllActiveVouchers: async (req, res) => {
            const Vouchers = await getDynamicVoucher({ _id: req.admin?._id }, true);
            res.status(200).json(Vouchers);
        },

        // Get all DeActive Vouchers
        getAllDeActiveVouchers: async (req, res) => {
            const Vouchers = await getDynamicVoucher({ _id: req.admin?._id }, false);
            res.status(200).json(Vouchers);
        },

        // Get all Vouchers
        getAllVouchers: async (req, res) => {
            const Vouchers = await getAllVouchers({ _id: req.admin?._id });
            res.status(200).json(Vouchers);
        },
