...
const orderController = () => {
    return {
        ...
        getCustomerSingleOrder: async (req, res) => {
            ...
            const order = await getSingleOrder({ $and: [{ _id: mongoose.Types.ObjectId(req.params?.id) }, { user_id: req.user?._id }] }, res);
        ...
        getCustomerAllOrders: async (req, res) => {
            const orders = await getAllOrders({ user_id: req.user?._id }, res)
        ...
        cancelOrder: async (req, res) => {
            ...
            const order = await getSingleOrder({ _id: mongoose.Types.ObjectId(req.params.id), user_id: req.user?._id }, res)
            if (!order) return error().resourceError(res, 'Sorry! This Order doest not exits or something wrong', 422);

            if (order.status !== 'PENDING') return error().resourceError(res, 'Sorry! You can not cancel this order right now', 422);

            const deletedOrder = await updateOrder({ _id: mongoose.Types.ObjectId(req.params.id) }, { status: 'CANCELED' }, res)
